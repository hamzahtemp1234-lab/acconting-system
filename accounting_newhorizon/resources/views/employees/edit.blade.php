...
<main class="flex-1 p-8 overflow-y-auto">
    <header class="flex justify-between items-center mb-8 pb-4 border-b border-gray-200">
        <h1 class="text-3xl font-extrabold text-primary">
            <i class="fas fa-pen-to-square ml-3 text-secondary"></i> تعديل موظف: {{ $employee->name }}
        </h1>
    ...
    <div class="max-w-5xl mx-auto bg-white rounded-xl shadow p-8">
        <form action="{{ route('employees.update', $employee) }}" method="POST" class="space-y-8">
            ...
            <div class="grid md:grid-cols-3 gap-6">
                <div>
                    <label class="block text-sm mb-2">رقم/رمز الموظف</label>
                    <input value="{{ $employee->code }}" class="w-full border rounded-lg px-4 py-2 bg-gray-50" readonly>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm mb-2">الاسم <span class="text-red-500">*</span></label>
                    <input name="name" value="{{ old('name', $employee->name) }}" maxlength="255" class="w-full border rounded-lg px-4 py-2" required>
            ...
            <div class="grid md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="account_id" class="w-full border rounded-lg px-4 py-2">
                        ...
                        <option value="{{ $a->id }}" @selected(old('account_id', $employee->account_id)==$a->id)>{{ $a->code }} - {{ $a->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input name="phone" value="{{ old('phone', $employee->phone) }}" maxlength="50" class="w-full border rounded-lg px-4 py-2">
                    @error('phone')<div class="text-red-500 text-sm">{{ $message }}</div>@enderror
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm mb-2">البريد الإلكتروني</label>
                    <input name="email" type="email" value="{{ old('email', $employee->email) }}" maxlength="100" class="w-full border rounded-lg px-4 py-2">
