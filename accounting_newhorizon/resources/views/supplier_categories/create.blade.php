...
<main class="flex-1 p-8 overflow-y-auto">
    <header class="flex justify-between items-center mb-8 pb-4 border-b border-gray-200">
        <h1 class="text-3xl font-extrabold text-primary">
            <i class="fas fa-plus ml-3 text-secondary"></i> إضافة تصنيف مورد
        </h1>
    ...
    <div class="max-w-4xl mx-auto bg-white rounded-xl shadow p-8">
        ...
        <form action="{{ route('supplier-categories.store') }}" method="POST" class="space-y-8">
            @csrf

            <div class="grid md:grid-cols-3 gap-6">
                <div>
                    <label class="block text-sm mb-2">الكود (يتولد تلقائيًا)</label>
                    <input name="code" type="text" value="{{ $nextCode ?? 'سيُولد تلقائيًا' }}" class="w-full border rounded-lg px-4 py-2 bg-gray-50" readonly>
                </div>
