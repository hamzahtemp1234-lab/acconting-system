...
<main class="flex-1 p-8 overflow-y-auto">
    <header class="flex justify-between items-center mb-8 pb-4 border-b border-gray-200">
        <h1 class="text-3xl font-extrabold text-primary">
            <i class="fas fa-sitemap ml-3 text-secondary"></i> إضافة قسم
        </h1>
        ...
        <form action="{{ route('departments.store') }}" method="POST" class="space-y-8">
            ...
            <div class="grid md:grid-cols-3 gap-6">
                <div>
                    ...
                    <select name="branch_id" id="branch_id" class="w-full border rounded-lg px-4 py-2" required>
                ...
                <div>
                    ...
                    <input id="dep_code"
                        value="{{ old('deb_code', $nextCode ?? '') }}"
                        class="w-full border rounded-lg px-4 py-2 bg-gray-50" placeholder="—" readonly>

                    @isset($nextCode)
                    <p class="text-xs text-gray-500 mt-1">المقترح: {{ $nextCode }}</p>
                    @endisset
                </div>
            ...
            <div class="grid md:grid-cols-3 gap-6">
                <div class="md:col-span-2">
                    ...
                    <select name="manager_id" class="w-full border rounded-lg px-4 py-2">
                        ...
                        <option value="{{ $emp->id }}" @selected(old('manager_id')==$emp->id)>{{ $emp->code }} - {{ $emp->name }}</option>
                        @endforeach
                    </select>
                    @else
                    <input name="manager_id" value="{{ old('manager_id') }}" class="w-full border rounded-lg px-4 py-2" placeholder="ID المدير (اختياري)">
                    @endisset
                    @error('manager_id')<div class="text-red-500 text-sm">{{ $message }}</div>@enderror
                    <p class="text-xs text-gray-500 mt-1">يمكن تركه فارغًا وتعيينه لاحقًا.</p>
                </div>
            </div>
