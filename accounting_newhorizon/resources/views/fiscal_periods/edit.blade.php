...
<main class="flex-1 p-8 overflow-y-auto">
    ...
    <header class="flex justify-between items-center mb-8 pb-4 border-b border-gray-200">
        <h1 class="text-3xl font-extrabold text-primary">
            <i class="fas fa-edit ml-3 text-secondary"></i> تعديل الفترة المالية
        </h1>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-xl shadow-2xl border-t-4 border-primary p-8">
            <form action="{{ route('fiscal-periods.update', $fiscalPeriod->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="fiscal_year_id" name="fiscal_year_id"
                            ...
                            <option value="{{ $year->id }}"
                                {{ $fiscalPeriod->fiscal_year_id == $year->id ? 'selected' : '' }}>
                                {{ $year->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" id="period_no" name="period_no" min="1" max="13"
                            value="{{ old('period_no', $fiscalPeriod->period_no) }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-secondary focus:border-secondary transition"
                            required>
                        @error('period_no')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    ...
                    <div>
                        ...
                        <input type="date" id="start_date" name="start_date"
                            value="{{ old('start_date', $fiscalPeriod->start_date) }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-secondary focus:border-secondary transition"
                            required>
                        @error('start_date')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    ...
                    <div>
                        ...
                        <input type="date" id="end_date" name="end_date"
                            value="{{ old('end_date', $fiscalPeriod->end_date) }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-secondary focus:border-secondary transition"
                            required>
                        @error('end_date')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    ...
                    <div class="flex items-center space-x-2 space-x-reverse">
                        <input type="checkbox" id="is_closed" name="is_closed" value="1"
                            ...
                            {{ $fiscalPeriod->is_closed ? 'checked' : '' }}>
