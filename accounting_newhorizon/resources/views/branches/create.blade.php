...
<main class="flex-1 p-8 overflow-y-auto">
    <header class="flex justify-between items-center mb-8 pb-4 border-b border-gray-200">
        <h1 class="text-3xl font-extrabold text-primary">
            <i class="fas fa-code-branch ml-3 text-secondary"></i> إضافة فرع
        </h1>
    ...
    <div class="max-w-5xl mx-auto bg-white rounded-xl shadow p-8">
        <form action="{{ route('branches.store') }}" method="POST" class="space-y-8">
            ...
            <div class="grid md:grid-cols-3 gap-6">
                <div>
                    <label class="block text-sm mb-2">رمز الفرع</label>
                    <input name="code" value="{{ old('code', $nextCode ?? '') }}" maxlength="20"
