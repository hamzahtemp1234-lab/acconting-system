...
<main class="flex-1 p-8 overflow-y-auto">
    <header class="flex justify-between items-center mb-8 pb-4 border-b border-gray-200">
        <h1 class="text-3xl font-extrabold text-primary">
            <i class="fas fa-cash-register ml-3 text-secondary"></i> إضافة صندوق
        </h1>
    ...
    <div class="max-w-5xl mx-auto bg-white rounded-xl shadow p-8">
        ...
        <form action="{{ route('cash-boxes.store') }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div class="grid md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="keeper_employee_id" class="w-full border rounded-lg px-4 py-2">
                        ...
                        <option value="{{ $k->id }}" @selected(old('keeper_employee_id')==$k->id)>{{ $k->code }} - {{ $k->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-3 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input name="notes" value="{{ old('notes') }}" class="w-full border rounded-lg px-4 py-2">
                </div>
                <div>
